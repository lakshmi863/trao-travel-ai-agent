...
import { Plane, Plus, MapPin, Wallet, Calendar, Trash2, Loader2, LogOut, CheckCircle2, RefreshCw } from 'lucide-react';
            ...
            <div className="space-y-3">
              ...
              {trips.map(t => (
                <div key={t._id} className="group relative">
                  <button onClick={() => setSelectedTrip(t)} className={`w-full p-4 rounded-2xl text-left border transition ${selectedTrip?._id === t._id ? 'bg-blue-600 border-blue-400 shadow-lg' : 'bg-slate-800 border-slate-700 hover:bg-slate-750'}`}>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                  <div>
                    <h2 className="text-5xl font-black text-white tracking-tight !text-white">{selectedTrip.destination}</h2>
                    <div className="flex flex-wrap gap-4 mt-4 text-slate-400 font-medium">
                      <span className="flex items-center gap-1.5 px-3 py-1 bg-slate-800 rounded-full border border-slate-700"><Calendar size={16} className="text-blue-500" /> {selectedTrip.durationDays} Days</span>
                      <span className="flex items-center gap-1.5 px-3 py-1 bg-slate-800 rounded-full border border-slate-700"><Wallet size={16} className="text-green-500" /> {selectedTrip.budgetTier} Budget</span>
                    </div>
                  </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {day.activities.map((act: any, i: number) => (
                          <div key={i} className="bg-slate-800/40 border border-slate-700/50 p-5 rounded-2xl group hover:border-blue-500/50 transition">
                            ...
                            <p className="text-sm text-slate-400 leading-relaxed !text-white">{act.description}</p>
                            {act.estimatedCostUSD > 0 && <p className="text-xs text-slate-500 mt-3 font-medium">Est. Cost: ${act.estimatedCostUSD}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
